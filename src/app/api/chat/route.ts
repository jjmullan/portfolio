...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    return new Response(
      `${STREAM_ERROR_PREFIX}요청 본문(JSON)이 올바르지 않습니다.`,
      {
        status: 400,
        headers: {
  ...
  if (!process.env.ANTHROPIC_API_KEY) {
    return new Response(
      `${STREAM_ERROR_PREFIX}서버 API 키가 설정되지 않았습니다.`,
      {
        status: 500,
        headers: {
          "Content-Type": "text/plain; charset=utf-8",
  ...
  try {
    ...
    return new Response(
      `${STREAM_ERROR_PREFIX}서버 프롬프트 파일을 불러오지 못했습니다.`,
      {
        status: 500,
        headers: {
          "Content-Type": "text/plain; charset=utf-8",
  ...
  try {
    ...
    return new Response(
      `${STREAM_ERROR_PREFIX}AI 스트림 초기화에 실패했습니다.`,
      {
        status: 502,
        headers: {
          "Content-Type": "text/plain; charset=utf-8",
    ...
    async start(controller) {
      ...
      try {
        ...
        if (error instanceof Anthropic.APIError) {
          if (error.status === 529)
            message = "현재 AI 서버가 혼잡합니다. 잠시 후 다시 시도해주세요.";
          else if (error.status === 429)
            message = "요청이 너무 많습니다. 잠시 후 다시 시도해주세요.";
          else if (error.status === 401)
            message = "API 인증에 실패했습니다. 관리자에게 문의해주세요.";
          else if (error.status === 400)
            message = "요청 형식 또는 모델 설정이 올바르지 않습니다.";
        }
